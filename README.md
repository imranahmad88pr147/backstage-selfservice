...
**# 11. Configure GitHub Organization, Production Reviewers, and Terraform State**

Before using the platform, configure the GitHub organization-specific settings below.

**## GitHub Organization and Reviewer Team**

The organization using this platform must create a GitHub team for the platform engineers who will review production deployments.

The team should:

1. Be created in the GitHub organization where the generated service repositories will be created.
2. Include the platform engineers responsible for reviewing production infrastructure changes.
3. Have **Read** access to the repositories generated by the platform.
4. Be used as the reviewer team for the GitHub `production` environment.

These team members are the reviewers for production deployments. Repository access and deployment approval are separate controls.

### Configure Repository Access

Set the team configuration in `app-config.yaml`:
...
platform:
  github:
    ...
    productionEnvironment:
      name: production
      reviewerTeamId: <your-team-id>
```

Replace `<your-github-org>` with the GitHub organization that owns the generated repositories.

Replace `<your-team-slug>` with the URL-friendly slug of the GitHub team that should have repository access.

Replace `<your-team-id>` with the numeric GitHub team ID of the reviewer team.

The `teamSlug` is used for repository access, while `reviewerTeamId` is used when configuring the GitHub `production` environment.

### GitHub Team Access

In the GitHub organization settings, give the reviewer team the required repository access. For this platform's default setup, the team should have **Read** access to generated repositories.

This allows platform engineers to inspect the repository and Terraform configuration while production deployment approval remains controlled by the GitHub `production` environment.

**## Configure the Shared Terraform State Bucket**

The generated service repositories use an S3 backend for Terraform remote state.

Each organization using this platform must provide its own shared Terraform state bucket.

Open:

```text
templates/healthcare-service/content/terraform/main.tf
```
...
Replace `your-bucket-name` with the name of the S3 bucket that your organization uses for shared Terraform remote state.
...
The state bucket should be managed by the organization operating the platform and should not reference another organization's bucket.

The S3 backend uses:

- `use_lockfile = true` for Terraform state locking.
- `encrypt = true` for server-side encryption of the state object.

Make sure the AWS identity used by the generated GitHub Actions workflow has the required permissions to access the state bucket.

**## Configure Platform GitHub Settings**
...
Use the same organization and reviewer team configured above.
...
platform:
  github:
    organization: my-healthcare-organization
    ...
    productionEnvironment:
      name: production
      reviewerTeamId: 12345678
```

Do not copy the example team ID into your installation. Use the ID of the team in your own GitHub organization.
...
**# 14. Configure the Service Repository**
...
For `prod`, the generated workflow uses the GitHub `production` environment. The configured reviewer team must approve the deployment before Terraform can apply the production plan.
...
**## Platform GitHub Configuration**

The platform-specific GitHub configuration is stored in `app-config.yaml`:
...
- `organization` is the GitHub organization where generated repositories are created.
- `repositoryAccess.teamSlug` identifies the GitHub team that should have repository access.
- `productionEnvironment.name` is the GitHub environment used for production deployments.
- `productionEnvironment.reviewerTeamId` identifies the team that reviews production deployments.

The reviewer team should contain the platform engineers responsible for production approvals.
...
The generated Terraform configuration uses the organization's shared S3 state bucket.
...
with the organization's Terraform state bucket before using the platform.
